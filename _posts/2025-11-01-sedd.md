...
related_posts: false
thumbnail: assets/img/language_diffusion.gif
...
This ratio tells us how likely we are to transition from the current token at position 

$$
i$$ to any other token
$$

j

$$
in the vocabulary. Why is this better? These ratios are the discrete analog of the score function
$$
...
In practice, we scale these matrices by a noise schedule 
...
, so 
$$
Q_t = \sigma(t) Q_{\text{absorb}}
$$

. This allows us to control how quickly the corruption happens. The forward transition probabilities can be computed analytically:
...
. To make this practical, the paper derives a denoising variant analogous to denoising score matching in continuous diffusion:
...
This is computationally efficient because we only need to evaluate 

$$
s_\theta(x)
$$

 once per sample (giving us all ratios for that position), and the conditional transition probabilities 
$$
\frac{p(y|x_0)}{p(x|x_0)}
$$
...
Since we've learned the ratios 

$$
s_\theta(x, t)_y \approx \frac{p_t(y)}{p_t(x)}
$$

, we can construct an approximate reverse process:
...
Let's walk through a simplified example using absorbing transitions with the sentence "cat in hat" (3 tokens). Suppose our vocabulary is {cat, in, hat, dog, the, MASK} with size 
...
, we apply the absorbing transition. Let's say we're at 
$$
t=0.5
$$

 with noise schedule giving 
$$
\sigma(0.5) = 1.5
$$
...
After sampling, we might get 

$$
x_{0.5} = [\text{MASK}, \text{in}, \text{MASK}]
$$

. Each token transitions independently, so "in" happened to stay unchanged while "cat" and "hat" both transitioned to MASK.
...
$$
\sigma_{0.5}^{0.5} = \sigma(0.5) - \sigma(0)
$$

 is the cumulative noise difference.
...
, successfully recovering the original text.
